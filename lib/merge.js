...
import { shouldApplyMeasurement } from './measure.js';

function mergeResourceFacts(target, incoming) {
  // Source precedence is shared with rescan/UI application: stronger browser
  // evidence wins, and byte provenance stays separate from format provenance
  // (a HEAD response never proves the page loaded that variant, so a known
  // content type is never overwritten here).
  if (shouldApplyMeasurement(target, incoming)) {
